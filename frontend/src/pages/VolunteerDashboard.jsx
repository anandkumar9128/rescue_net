...
function TaskOfferModal({ offer, onAccept, onReject }) {
  const [countdown, setCountdown] = useState(
    Math.floor((offer.timeout_ms || 120000) / 1000),
  );
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="glass-card p-8 max-w-sm w-full text-center animate-slide-up border-brand-500/40">
        ...
        <p className="text-slate-400 text-sm mb-1">
          {offer.need_type} Emergency
        </p>
        <p className="text-slate-500 text-xs mb-1">
          👥 {offer.total_people} people · {offer.max_severity} severity
        </p>
        {offer.required_volunteers > 1 && (
          <p className="text-brand-400 text-xs font-medium mb-3">
            🤝 Team Task: {offer.required_volunteers} volunteers requested
          </p>
        )}
        <p className="text-slate-600 text-xs mb-6">
          📍 {offer.location?.lat?.toFixed(4)},{" "}
          {offer.location?.lng?.toFixed(4)}
        </p>
        ...
        <div className="flex gap-3">
          <button onClick={onReject} className="btn-ghost flex-1 text-sm">
            Reject
          </button>
          <button onClick={onAccept} className="btn-primary flex-1 text-sm">
            Accept Task
          </button>
        </div>
      </div>
...
function TaskCard({ task, onStatusUpdate }) {
  ...
  const handleNext = async () => {
    ...
    setUpdating(true);
    try {
      await api.patch(`/volunteers/assignments/${task._id}/status`, {
        status: next,
      });
  ...
  return (
    <div
      className={`glass-card p-5 space-y-4 ${task.status !== "Completed" && task.status !== "Cancelled" ? "border-brand-500/20" : ""}`}
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-semibold text-white">
              {cluster?.need_type || "Unknown"}
            </div>
            <div className="text-slate-500 text-xs">{task.ngo_id?.name}</div>
          </div>
        </div>
        <span
          className={`badge border ${STATUS_COLORS[task.status] || "badge bg-slate-500/15 text-slate-400"}`}
        >
          {task.status}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 text-xs text-slate-400">
        ...
        {task.required_volunteers > 1 && (
          <div className="col-span-2 text-brand-400 font-medium">
            🤝 Team Task: You are 1 of {task.required_volunteers} volunteers
          </div>
        )}
      </div>
...
export default function VolunteerDashboard() {
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      await api.patch(`/volunteers/${volunteerId}/status`, {
        status: newStatus,
      });
  ...
  useEffect(() => {
    const activeTask = tasks.find((t) =>
      ["En Route", "On Task", "Volunteer Assigned"].includes(t.status),
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div>
          <h3 className="font-display font-semibold text-white mb-3">
            ...
            {activeTasks.length > 0 && (
              <span className="badge bg-brand-500/20 text-brand-400 ml-2">
                {activeTasks.length}
              </span>
            )}
          </h3>
          {loading ? (
            ...
            <div className="glass-card p-8 text-center">
              ...
              <div className="text-slate-600 text-xs mt-1">
                Set your status to Available to receive tasks
              </div>
            </div>
          ) : (
        ...
        {completedTasks.length > 0 && (
          <div>
            <h3 className="font-display font-semibold text-white mb-3 text-sm text-slate-400">
              Completed Tasks
            </h3>
